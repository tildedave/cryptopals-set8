...
class RingContext:
    ...
    def matrix_multiply_naive(self, a: Matrix, b: Matrix) -> Matrix:
        ...
        for i in range(0, len(a)):
            for j in range(0, len(b)):
                ...
                for n in range(0, len(a)):
                    m = self.element_mult(a[i][n], b[n][j])
                    sum = self.element_add(sum, m)
                c[i][j] = sum
    ...
    def matrix_add(self, a: Matrix, b: Matrix) -> Matrix:
        ...
        for i in range(0, len(a)):
            for j in range(0, len(a)):
                c[i][j] = self.element_add(a[i][j], b[i][j])

        return c

    def matrix_multiply_divide_and_conquer(self,
                                           a: Matrix,
                                           b: Matrix,
                                           ) -> Matrix:
        """
        ...
        c_11 = self.matrix_add(
            self.matrix_multiply_divide_and_conquer(a_11, b_11),
            self.matrix_multiply_divide_and_conquer(a_12, b_21))
        c_12 = self.matrix_add(
            self.matrix_multiply_divide_and_conquer(a_11, b_12),
            self.matrix_multiply_divide_and_conquer(a_12, b_22))
        c_21 = self.matrix_add(
            self.matrix_multiply_divide_and_conquer(a_21, b_11),
            self.matrix_multiply_divide_and_conquer(a_22, b_21))
        c_22 = self.matrix_add(
            self.matrix_multiply_divide_and_conquer(a_21, b_12),
            self.matrix_multiply_divide_and_conquer(a_22, b_22))
...
def test_matrix_multiply():
    ...
    assert IntegerContext.matrix_multiply_naive(b, a) == [[3, 2], [4, 3]]
    assert IntegerContext.matrix_multiply_divide_and_conquer(b, a) == \
        [[3, 2], [4, 3]]

    c = [[2, 7, 3], [1, 5, 8], [0, 4, 1]]
    d = [[3, 0, 1], [2, 1, 0], [1, 2, 4]]

    assert IntegerContext.matrix_multiply_naive(c, d) == \
